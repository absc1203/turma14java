...
public class bancoG3 {
	...
	public static void main(String[] args) {
		...
		String nomeCliente[] = new String[] { "Allen de Lima Vieira", "André de Brito Silva da Costa",
				...
				"Francisco José Pires", "Gabriel Enrique Cabral Silva", "Gabriel Sérgio Nascimento Santos Gonçalves",
		...
		char sexo[] = new char[] { 'M', 'M', 'F', 'F', 'M', 'M', 'M', 'M', 'M', 'M', 'M', 'M', 'M', 'F', 'F', 'M', 'M',
				'M', 'M', 'M', 'M', 'M', 'F', 'M', 'F', 'F', 'M', 'F', 'M', 'F', 'M', 'M', 'M', 'M', 'M', 'F', 'F', 'M',
				'F', 'M' };
	...
	static void contaCorrente(double saldo, double saldos[], int talao[], int totalTalao[], double movimento[],
			char tipoMovimento[], int conta) {
			...
			for (int i = 0; i < 10; i++) {
				...
				if (opcao == '3') {
					...
					if (valor > limiteEmprestimo || valor < 0) {
						System.out.println(
								"Valor informado é maior que o saldo do emprestimo atual digite um número válido");
					} else {
						limiteEmprestimo = limiteEmprestimo - valor;
						valorAtual = limiteEmprestimo;
						saldo = saldo + valor;
						System.out.println("operação realizada com sucesso.\n SALDO ATUAL: " + saldo
								+ " seu limite emprestimo é: " + valorAtual);
